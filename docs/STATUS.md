...
**上次整理：2026-09-18（P3 第二轮：ACP 接线跑通并真跑，量出每次调用固定开销 ≈13k）**
...
**下一步**
1. **P3 第三**：`entity` / `relation` 表 + 写入口（按 (name,type) / (src,dst,keywords) 归并、
   每条来源各占一行、不丢出处）→ 把 `vault extract` 的产物灌进去 → 再跑 20 篇看质量（`OPEN.md` #24）
