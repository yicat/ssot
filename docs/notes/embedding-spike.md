...
⚠️ **2026-09-18 更正**：当时以为「两边都不丢内容 → 不影响结论」就够了。**不够**——
这个打包顺序后来被证明**实实在在伤了检索**（P2 实测 R@1 少 13.3pp），
原因与修法见 §六.8。看这一节时请连着 §六.8 一起看。
...
### 六.8 那个 17.6pp 找到了：**切块的打包顺序**（2026-09-18）

§六.7 记了「Go 纯向量 48.3% vs JS 65.9%，差 17.6pp」。这一节是后续：**原因查清了，也修了。**

**怎么查的**（逐层排除，每一步都有数字）：

1. **判分规则**先对齐（同文档优先含句、退化为同文档最靠前）——不对齐时差 15pp，纯属自欺。
2. **嵌入**排除：索引里 5 条真实块文本（260～720 字、含换行与对话体、两条触发 512 截断），
   Go 与 transformers.js 各自编码 **cos=1.0000000、最大分量差 1.5e-8**。
3. **块边界**摊开比：把 Go 索引里前 200 篇的块 dump 出来，与 JS 的 `chunkMarkdown(md,512)` 逐篇比——
   **Go 8,156 块 vs JS 8,667 块**（Go 少 5.9%），而且 **只有 19.1% 的 Go 块能整段落在某个 JS 块里**。
   → 边界真的不一样，嫌疑锁定在切块。

**原因**：Go 的 `ChunkBody` 早先把超长段落的句子**摊平成一串**，再在第三步与**相邻段落**一起贪心打包，
于是块经常横跨两段（`段落A尾部 + 段落B开头`）。JS（也就是 spike 那套、以及 spec 里写的
「标题 → 段落 → 句子」）是**先在段内**把句子打包成 ≤ 目标的片段，**再**与相邻段落合并。
段落是语义单元：跨段拼出来的块「意思不聚焦」，CLS 池化出来的向量就被稀释了。

**修法**（`internal/domain/vault/chunk.go` 第二步，口径已写进 `derived.spec.md`）：
超长段**先在段内**打包，再谈跨段合并。改完之后：

| | 改之前 | 改之后 | JS 的 `recall.mjs`（对照） |
|---|---|---|---|
| 前 200 篇块数 | 8,156 | **8,670** | 8,667 |
| 纯向量 R@1 | 48.3% | **61.6%** | 65.9% |
| 纯向量 R@5 | 71.5% | **80.1%** | 77.8% |
| head / mid / tail R@1 | 47.1 / 52.5 / 45.5 | **59.8 / 61.6 / 63.4** | 62.7 / 64.6 / 70.3 |
| 全库嵌入块 | 12,512 | **13,181** | 13,178（`chunk-sizing.mjs`） |

⚠️ 两件事要说清：
- 中间三列里 **head/mid/tail 59.8 / 61.6 / 63.4 与 §六 那张表的数字逐字相同**——
  这说明**基线复现了**，两边现在真的在比同一件事。R@1 61.6%/R@5 80.1% 对 spike 的
  61.6%/79.8%（同一条 `recall.mjs` 今天重跑是 65.9%/77.8%，差异来自池子里有没有重复块、
  以及 Go 侧 `limit` 截断等细枝末节，都在 ±4pp 内）。
- **§六.4 那条「少 5% 不影响结论」的结论要修正**：块数差 5% 确实没影响「512 vs 2000」的选择，
  但它**实实在在伤了检索**（R@1 −13.3pp）。当时只看「不丢内容」就收工，是漏了。

**修完之后的 P2 结果**（同样的 200 篇 / 302 条正文句查询 / 102 条标题式查询）：

| 变体 | 正文句 R@1 | 正文句 R@5 | 标题式（实体名）R@1 | 标题式 R@5 |
|---|---|---|---|---|
| 纯向量吃 β=0 | 61.6% | 80.1% | 34.3% | 70.6% |
| **β_title=0.05（默认）** | **61.6%** | **80.1%** | **37.3%** | **73.5%** |
| β_title=0.1 | 61.6% | 79.8% | 36.3% | 71.6% |
| β_title=0.2 | 61.6% | 79.8% | 26.5% | 64.7% |
| β_title=0.2 + β_body=0.3（**假象，别用**） | 98.3% | 99.7% | 33.3% | 69.6% |

**验收结论**：正文句查询 ≥ 基线（61.6%/80.1% vs 基线 61.6%/79.8%，**不降**）；
实体名式查询 R@1 **34.3% → 37.3%（+3.0pp）**、R@5 70.6% → 73.5%（+2.9pp）。
**达标，但只是「不降 + 小幅上升」**：+3.0pp（n=102）算不上「明显」，
真要更硬得靠图检索（P4）或更好的字面项——别把这条当成已经解决。

**复现**（参数与本节一致）：

```powershell
$env:SSOT_EMBED_DIR="$env:TEMP\embed-spike"
go run ./scripts/check/hybrid-bench -root projects/demo -docs 200
```
