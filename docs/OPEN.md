...
| 33 | `vault_search` 的 **50 条上限**把模型逼去翻表（这条正是 #31 的起因）：`Index.Search` 在 `limit<=0` 时给 50；模型传 `limit=500` 也仍是那 50 条。**待复核**：是上层截断、还是 `limit` 没透传？要么修好 `limit`，要么把上限写进工具说明让模型别绕路 | 待定 | P4 / 与 #31 连着 | `internal/infrastructure/vaultindex/index.go` `Search` |
