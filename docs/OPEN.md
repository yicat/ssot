...
| 31 | **`table_query` 能查派生层的表**：它跑在**索引库**上（`vaultapp.QueryTables` → `index.Query`），所以 `chunk`（含块正文！）/`embedding`/`entity`/`relation` 全都 reachable——**真跑时模型就是这么干的**：`vault_search` 撞到 50 条上限后，它转头 `SELECT ... FROM chunk` 去凑完整清单。这跟「派生层沉到水下」（ADR 0007）不一致，也让检索（向量/图）变成可选。选项：给 `table_query` **白名单**（只放数据表 + `docs` 的 front matter 字段）／只放行 `SELECT` 且禁止查那五张内部表／维持现状（当它是给人用的调试口） | 待定 | 与 ADR 0007、P4 相关 | `vaultapp.QueryTables`、`vaultindex/index.go` 的 `Query` |
| 32 | **agent 手上的 `vault_search` 还是关键词检索**：MCP 的 `vault_search` 走 `vaultapp.Search` → `index.Search`，是 SQL `LIKE`；我们做的**向量 / 混合 / 图检索**（`vaultapp/vector.go`、`graph.go`、`Searcher`）目前只在 CLI（`vault vector`、`vault find`）与界面里用。所以现在「关掉 grep」的收益只是**范围受限**，agent 并没有真用上 RAG。要不要把 `vault_search` 接成 hybrid（P4 的 `--mode`）？ | 待定 | P4 | `internal/mcp/tools.go` 的 `vault_search`、`vaultapp/vector.go`、`graph.go` |
| 33 | `vault_search` 的 **50 条上限**把模型逼去翻表：`Index.Search` 在 `limit<=0` 时给 50，而模型传 `limit=500` 时拿到的仍是那 50 条（待复核：是上层截断还是 `limit` 没透传）。要么把 `limit` 修好，要么把上限写进工具说明让模型别绕路 | 待定 | P4 / 与 #31 连着 | `internal/infrastructure/vaultindex/index.go` `Search` |
