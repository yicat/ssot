...
| 24 | 抽取的**提示词与实体类型词表没定稿**（spec §六.4）：现在有一版起点（LightRAG 结构 + 我们的 doc/行号要求 + 9 类词表），要用跑批产出与坏例来调。**已有证据（3 篇真跑）**：`Other` 最多（26/92 行），还有把表名（`增益减益.csv`）当实体、把命令示例（`ssot vault`）当关系端点的——提示词要明确「不要文件名/命令示例」**（已加，17/20 篇复查：坏例只是换了类型没消失 → 改为在代码里做确定性过滤）** | 待定 | P3 质量 | `internal/infrastructure/vextract/prompt.go`、`extraction-spike.md`「P3 第三轮」 |
