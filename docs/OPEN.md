...
| 20 | **Go 侧纯向量 48.3%/71.5%，JS 路径同 vault 同查询是 65.9%/77.8%——差 17.6pp 没定位**。嵌入已排除（真实块上 cos=1.0）；嫌疑在**块边界**（Go 40.8 块/篇 vs JS ≈33.9）或池构造 | **阻塞** | P2 验收（达标才有意义） | `embedding-spike.md` §六.7 |
| 21 | 标题项权重：实测 β=0.05 最好（标题查询 R@1 +3.0pp），β=0.2 反而 −6.8pp——与 spike 的单调结论不一致。默认已定 0.05，但**样本只有 102 条查询**，结论还不硬 | 待定 | P2 定稿 | `domain/vault/rank.go` |
