...
| 23 | **抽取的模型调用走哪条路**：`dsh` headless 的任务文本是**命令行参数**，撞 Windows 32,767 字符上限 → 一次只放得下 5～6 块，与「必须多块合一次」冲突。**计划走 ACP**（协议传提示词，无此上限），但 run3 的 token 数是 headless 量的，换路后**要重测** | 待定 | P3 接线 | `extraction-spike.md`「命令行长度」一节 |
| 24 | 抽取的**提示词与实体类型词表没定稿**（spec §六.4）：现在有一版起点（LightRAG 结构 + 我们的 doc/行号要求 + 9 类词表），要用跑批产出与坏例来调 | 待定 | P3 质量 | `internal/infrastructure/vextract/prompt.go` |
