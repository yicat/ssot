...
| 34 | **「没留痕」的措辞在三种情况下会说不准**：`vaultgit.Commit` 对「路径写错」「删掉了一个从未跟踪的文件」「文件被 .gitignore 忽略」都返回「没变化」，于是 `version()` 一律说成「文件内容没有变化，没有产生新提交」。加单测时发现的（`TestCommitOfMissingUntrackedPathIsSilentNoOp` 先钉住了现状）。要分开得让调用方参与判断：给 `Repo` 加 `Tracked(rel)`，由它（知道是写还是删）组措辞。**不擅自改契约**（这一层只有 `version()` 一个调用方，但它关系到「留痕」这条 spec） | 待定 | — | `internal/infrastructure/vaultgit/vaultgit.go`、`vaultapp/stat.go` 的 `version` |
